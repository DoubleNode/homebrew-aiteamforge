...
def get_kanban_tmp_dir(team_id: str) -> Path:
    ...
    Prefers KANBAN_TMP_DIR env var (set by startup script) over the hardcoded
    mapping, so it works on any install — not just dev-team.
    ...
    """
    # Startup script passes the resolved path via env var
    env_tmp = os.environ.get("KANBAN_TMP_DIR", "")
    if env_tmp:
