...
# ---------------------------------------------------------------------------
# System telemetry collectors (XACA-1091-002 / XACA-1091-003)
#
# Frozen field contract: kanban/plans/XACA-1091/CONTRACT-system-block.md
# Measured cost budget:  kanban/plans/XACA-1091/FINDINGS-001-cost-budget.md
#
# COLLECTOR FUNCTIONS ONLY. Nothing in this file calls them yet -- wiring
# them into build_payload()'s "system" block is XACA-1091-004, deliberately
# deferred: XACA-1031 (PR #818) is landing its own change to build_payload()
# at the same time, and touching that function here would create a needless
# merge conflict with a PR already in flight. Do not "helpfully" wire these
# in early.
#
# THE BASH 3.2 / 5.x TRAP THAT MOTIVATES THE STYLE BELOW.
# This script's shebang is `#!/usr/bin/env bash` (PATH bash, 5.3.3 measured
# on this host) but it is actually RUN by launchd/cron under `/bin/bash`
# (macOS's shipped 3.2.57). Under `set -euo pipefail` (top of file), a bare
# `local v` is empty-but-set on 3.2 and flat-out UNSET on 5.x -- so the exact
# same line is silent under one shell and an "unbound variable" abort under
# the other. XACA-1031 shipped this defect hours before this subitem was
# written and it deleted the host from Fleet Monitor entirely (build_payload
# aborted before emitting anything). EVERY `local` below therefore carries an
# explicit initializer, with no exceptions, and every external command is
# guarded by `command -v ... || return 0` so a missing binary degrades the
# collector to "nothing collected" instead of aborting the reporter.
#
# No `declare -A` anywhere (bash 3.2 has none) -- both maps below are
# newline-delimited "key=value" strings, following the _LCARS_LIVE_MAP
# precedent above.
#
# ABSENCE CONVENTION (contract §3): a value that cannot be collected is
# OMITTED by the getters below (they print nothing and return 0) -- never
# `null`, `""` as a sentinel, or `-1`. A collected zero (e.g. swap_used_bytes
# 0 meaning "not swapping") is real data and IS returned normally; only a
# failed parse/missing command yields the omitted (empty-string) shape. The
# emit step (XACA-1091-004) is responsible for turning "getter printed
# nothing" into "key absent from the JSON".

# Trim leading/trailing whitespace without spawning a subprocess. Portable
# parameter-expansion idiom -- no extglob, works on bash 3.2 and 5.x alike.
_system_trim() {
    local s="$1"
    s="${s#"${s%%[![:space:]]*}"}"
    s="${s%"${s##*[![:space:]]}"}"
...
# ---------------------------------------------------------------------------
# STATIC tier (XACA-1091-002): os_version, os_build, os_name, model, arch,
# cores, total_ram, boot_time. Facts that cannot change without a reboot --
# collected ONCE per reporter run and cached, mirroring the
# _lcars_build_live_map guard shape (module-level global + a _BUILT boolean
# so repeat calls are free). Measured ~18ms one-time per FINDINGS-001; not
# part of the recurring per-cycle budget by construction.
_SYSTEM_STATIC_MAP=""
_SYSTEM_STATIC_BUILT=false

_system_build_static_map() {
    if [ "$_SYSTEM_STATIC_BUILT" = true ]; then
        return 0
    fi
    _SYSTEM_STATIC_BUILT=true
    _SYSTEM_STATIC_MAP=""

    # sw_vers: os_name, os_version, os_build. Bare `sw_vers` returns all three
    # in ONE call; per FINDINGS-001 a single-field form (`-productVersion`
    # etc.) costs the same per call as the bare 3-field form, so three calls
    # would triple the static-tier cost for zero benefit.
    if command -v sw_vers >/dev/null 2>&1; then
        local sw_out="" sw_line="" sw_val=""
        sw_out="$(sw_vers 2>/dev/null || true)"
        if [ -n "$sw_out" ]; then
            while IFS= read -r sw_line; do
                case "$sw_line" in
                    ProductName:*)
                        sw_val="$(_system_trim "${sw_line#ProductName:}")"
                        [ -n "$sw_val" ] && _SYSTEM_STATIC_MAP="${_SYSTEM_STATIC_MAP}os_name=${sw_val}
"
                        ;;
                    ProductVersion:*)
                        sw_val="$(_system_trim "${sw_line#ProductVersion:}")"
                        [ -n "$sw_val" ] && _SYSTEM_STATIC_MAP="${_SYSTEM_STATIC_MAP}os_version=${sw_val}
"
                        ;;
                    BuildVersion:*)
                        sw_val="$(_system_trim "${sw_line#BuildVersion:}")"
                        [ -n "$sw_val" ] && _SYSTEM_STATIC_MAP="${_SYSTEM_STATIC_MAP}os_build=${sw_val}
"
                        ;;
                esac
            done <<_SYSTEM_SWVERS_EOF
$sw_out
_SYSTEM_SWVERS_EOF
        fi
    fi
    ...
    if command -v uname >/dev/null 2>&1; then
        local arch_val=""
        arch_val="$(uname -m 2>/dev/null || true)"
        if [ -n "$arch_val" ]; then
            _SYSTEM_STATIC_MAP="${_SYSTEM_STATIC_MAP}arch=${arch_val}
"
        fi
    fi
...
_SYSTEM_SYSCTL_EOF
            ...
            [ -n "$sc_model" ] && _SYSTEM_STATIC_MAP="${_SYSTEM_STATIC_MAP}model=${sc_model}
"

            case "$sc_mem" in
                ''|*[!0-9]*) ;;
                *) _SYSTEM_STATIC_MAP="${_SYSTEM_STATIC_MAP}total_ram=${sc_mem}
" ;;
            esac

            case "$sc_cpu" in
                ''|*[!0-9]*) ;;
                *) _SYSTEM_STATIC_MAP="${_SYSTEM_STATIC_MAP}cores=${sc_cpu}
" ;;
            esac

            # kern.boottime prints as: { sec = 1787581393, usec = 123456 } <date>
            # Extract ONLY the sec value -- contract requires epoch SECONDS.
            if [ -n "$sc_boot_raw" ]; then
                local sc_boot_sec=""
                sc_boot_sec="${sc_boot_raw#*sec = }"
                sc_boot_sec="${sc_boot_sec%%,*}"
                sc_boot_sec="$(_system_trim "$sc_boot_sec")"
                case "$sc_boot_sec" in
                    ''|*[!0-9]*) ;;
                    *) _SYSTEM_STATIC_MAP="${_SYSTEM_STATIC_MAP}boot_time=${sc_boot_sec}
" ;;
                esac
            fi
        fi
...
# Looks up one "key=value" record from _SYSTEM_STATIC_MAP. Prints the value,
# or nothing at all if the key was never collected (absence convention --
# caller/emit-step treats empty output as "omit this key", never as "0").
# Always exits 0.
_system_static_get() {
    local key="$1"
    [ -n "$key" ] || return 0
...
# ---------------------------------------------------------------------------
# VOLATILE tier (XACA-1091-003): memory (used/total/pressure_percent),
# swap_used_bytes, disk (used/free/percent), load_average. Sampled every
# reporter cycle (60s). Per FINDINGS-001 the affordable set is ONE combined
# `sysctl -n vm.swapusage vm.loadavg` call, plus `vm_stat`, plus `df -k /` --
# measured ~9-12ms per cycle, about 0.02% of the 60s window. `uptime` and
# `memory_pressure` are deliberately excluded (redundant / fragile prose
# output -- see FINDINGS-001 "Commands recommended AGAINST").
#
# UNLIKE the static map, this is rebuilt EVERY call -- no _BUILT guard -- so
# each 60s cycle gets a fresh sample. It still batches all three commands
# into ONE pass per invocation so a single collection cycle forks each
# command exactly once, however many of the getters below get called.
_SYSTEM_VOLATILE_MAP=""

_system_collect_volatile() {
    _SYSTEM_VOLATILE_MAP=""

    # --- swap + load average: one combined sysctl call -----------------
    if command -v sysctl >/dev/null 2>&1; then
        local vs_out="" vs_swap_line="" vs_load_line="" vs_n=0 vs_line=""
        vs_out="$(sysctl -n vm.swapusage vm.loadavg 2>/dev/null || true)"
        if [ -n "$vs_out" ]; then
            while IFS= read -r vs_line; do
                vs_n=$((vs_n + 1))
                case "$vs_n" in
                    1) vs_swap_line="$vs_line" ;;
                    2) vs_load_line="$vs_line" ;;
                esac
            done <<_SYSTEM_VMSC_EOF
$vs_out
_SYSTEM_VMSC_EOF

            # swap_used_bytes -- ABSOLUTE BYTES, never a percentage (contract
            # §4, non-negotiable: macOS grows the swapfile on demand, so
            # `total` is not a cap and a percentage against it reads
            # 72-97% on a healthy machine). Parse the "used = N<unit>" token;
            # `vm.swapusage` may append trailing annotations (e.g.
            # "(encrypted)") which this extraction ignores.
            if [ -n "$vs_swap_line" ]; then
                case "$vs_swap_line" in
                    *used\ =*)
                        local vs_used_tok="" vs_num="" vs_suffix="" vs_mult=1 vs_bytes=""
                        vs_used_tok="${vs_swap_line#*used = }"
                        vs_used_tok="${vs_used_tok%% *}"
                        case "$vs_used_tok" in
                            *[GgMmKk])
                                vs_suffix="${vs_used_tok: -1}"
                                vs_num="${vs_used_tok%?}"
                                ;;
                            *)
                                vs_suffix=""
                        ...
                        case "$vs_suffix" in
                            [Kk]) vs_mult=1024 ;;
                            [Mm]) vs_mult=1048576 ;;
                            [Gg]) vs_mult=1073741824 ;;
                            *) vs_mult=1 ;;
                        esac
                        case "$vs_num" in
                            ''|*[!0-9.]*) ;;
                            *)
                                vs_bytes="$(awk -v n="$vs_num" -v m="$vs_mult" 'BEGIN{printf "%.0f", n*m}' 2>/dev/null || true)"
                                case "$vs_bytes" in
                                    ''|*[!0-9]*) ;;
                                    *) _SYSTEM_VOLATILE_MAP="${_SYSTEM_VOLATILE_MAP}swap_used_bytes=${vs_bytes}
" ;;
                                esac
                                ;;
                        esac
            ...
            # load_average -- 3-element float array (1m, 5m, 15m). Strip the
            # `{ ... }` braces; stored as one space-separated record (the
...
"
    ...
    # --- memory: vm_stat, page counts x page size -----------------------
    # used = active + wired + compressed pages (per XACA-1091-003 spec).
    # "compressed" is Pages-occupied-by-compressor (physical pages the
    # compressor pool actually holds), not Pages-stored-in-compressor (the
    # pre-compression logical-page-equivalent count, which is far larger and
    # would overstate real memory use).
    if command -v vm_stat >/dev/null 2>&1; then
        local vm_out="" vm_line="" vm_page_size=0 vm_active=0 vm_wired=0 vm_compressed=0
        local vm_have_size=false vm_have_active=false vm_have_wired=false vm_have_compressed=false
            ...
            while IFS= read -r vm_line; do
                case "$vm_line" in
                    Mach\ Virtual\ Memory\ Statistics:*)
                        local vm_ps=""
                        vm_ps="${vm_line#*page size of }"
                        vm_ps="${vm_ps%% bytes*}"
                        case "$vm_ps" in
                            ''|*[!0-9]*) ;;
                            *) vm_page_size="$vm_ps"; vm_have_size=true ;;
                        esac
                        ;;
                    "Pages active:"*)
                        local vm_v=""
                        vm_v="$(_system_trim "${vm_line#Pages active:}")"
                        vm_v="${vm_v%.}"
                        case "$vm_v" in
                            ''|*[!0-9]*) ;;
                            *) vm_active="$vm_v"; vm_have_active=true ;;
                        esac
                        ;;
                    "Pages wired down:"*)
                        local vm_v2=""
                        vm_v2="$(_system_trim "${vm_line#Pages wired down:}")"
                        vm_v2="${vm_v2%.}"
                        case "$vm_v2" in
                            ''|*[!0-9]*) ;;
                            *) vm_wired="$vm_v2"; vm_have_wired=true ;;
                        esac
                        ;;
                    "Pages occupied by compressor:"*)
                        local vm_v3=""
                        vm_v3="$(_system_trim "${vm_line#Pages occupied by compressor:}")"
                        vm_v3="${vm_v3%.}"
                        case "$vm_v3" in
                            ''|*[!0-9]*) ;;
                            *) vm_compressed="$vm_v3"; vm_have_compressed=true ;;
                        esac
                        ;;
                esac
            done <<_SYSTEM_VMSTAT_EOF
$vm_out
_SYSTEM_VMSTAT_EOF

            if [ "$vm_have_size" = true ] && [ "$vm_have_active" = true ] && \
               [ "$vm_have_wired" = true ] && [ "$vm_have_compressed" = true ]; then
                local vm_used_bytes=""
                vm_used_bytes=$(( (vm_active + vm_wired + vm_compressed) * vm_page_size ))
                _SYSTEM_VOLATILE_MAP="${_SYSTEM_VOLATILE_MAP}memory_used=${vm_used_bytes}
"

                # memory.total mirrors total_ram (contract §2). Reuse the
                # cached static value instead of re-forking sysctl for it --
                # the static builder's own _BUILT guard makes this call free
                # if -002's collector already ran this process lifetime.
                local vm_total_ram=""
                vm_total_ram="$(_system_static_get total_ram)"
                case "$vm_total_ram" in
                    ''|*[!0-9]*) ;;
                    *)
                        _SYSTEM_VOLATILE_MAP="${_SYSTEM_VOLATILE_MAP}memory_total=${vm_total_ram}
"
                        # pressure_percent: used bytes as a percentage of
                        # total physical RAM, 0-100. Simple and bounded by
                        # construction (used <= total), and reuses numbers
                        # already computed above rather than a second,
                        # separately-sourced "pressure" metric.
                        local vm_pct=""
                        vm_pct="$(awk -v u="$vm_used_bytes" -v t="$vm_total_ram" 'BEGIN{ if (t > 0) printf "%.0f", (u * 100.0) / t }' 2>/dev/null || true)"
                        case "$vm_pct" in
                            ''|*[!0-9]*) ;;
                            *) _SYSTEM_VOLATILE_MAP="${_SYSTEM_VOLATILE_MAP}memory_pressure_percent=${vm_pct}
" ;;
                        esac
                        ;;
                esac
    ...
    # --- disk: df -k / ---------------------------------------------------
    # APFS gotcha (contract §5a): `df`'s TOTAL column is the shared container
    # size, NOT used+free -- `used / total` under-reports by ~20x on this
    # host's own measurement. disk.percent is therefore computed as
    # used / (used + free), matching df's OWN capacity column, and NEVER as
    # used / total.
    if command -v df >/dev/null 2>&1; then
        local df_out="" df_used_kb="" df_avail_kb="" df_fields=""
        df_out="$(df -k / 2>/dev/null || true)"
        if [ -n "$df_out" ]; then
            # Row 2 (row 1 is the header): Filesystem 1024-blocks Used
            # Available Capacity iused ifree %iused Mounted-on. Used is
            # field 3, Available is field 4. ONE awk call for both fields
            # (not two) -- this function already forks 3 processes per
            # cycle for sysctl/vm_stat/df; no reason to make it 4 for a
            # single-line extraction that fits in one call.
            df_fields="$(printf '%s\n' "$df_out" | awk 'NR==2{print $3, $4}')"
            df_used_kb="${df_fields%% *}"
            df_avail_kb="${df_fields#* }"
            case "$df_used_kb" in
                ''|*[!0-9]*) ;;
                *)
                    case "$df_avail_kb" in
                        ''|*[!0-9]*) ;;
                        *)
                            local df_used_bytes="" df_free_bytes="" df_pct=""
                            df_used_bytes=$(( df_used_kb * 1024 ))
                            df_free_bytes=$(( df_avail_kb * 1024 ))
                            _SYSTEM_VOLATILE_MAP="${_SYSTEM_VOLATILE_MAP}disk_used=${df_used_bytes}
disk_free=${df_free_bytes}
"
                            df_pct="$(awk -v u="$df_used_bytes" -v f="$df_free_bytes" 'BEGIN{ t = u + f; if (t > 0) printf "%.0f", (u * 100.0) / t }' 2>/dev/null || true)"
                            case "$df_pct" in
                                ''|*[!0-9]*) ;;
                                *) _SYSTEM_VOLATILE_MAP="${_SYSTEM_VOLATILE_MAP}disk_percent=${df_pct}
" ;;
                            esac
                            ;;
                    esac
...
# Looks up one "key=value" record from _SYSTEM_VOLATILE_MAP. Prints the
# value, or nothing if that key could not be collected this cycle (absence
# convention -- see header comment). Does NOT trigger collection itself --
# callers must call _system_collect_volatile() once per cycle first, so N
# getter calls cost one collection pass, not N. Always exits 0.
_system_volatile_get() {
    local key="$1"
    [ -n "$key" ] || return 0
...
build_payload() {
    ...
    # XACA-1091-004: prime the system-telemetry caches in THIS scope too, for
    # exactly the same reason as _lcars_build_live_map above. Both
    # _system_build_static_map and _system_collect_volatile set MODULE-LEVEL
    # globals (_SYSTEM_STATIC_MAP / _SYSTEM_VOLATILE_MAP); a fork created by
    # `$( )` command substitution gets its own COPY of the process and any
    # collection done inside that subshell is discarded the instant the
    # subshell exits. Calling them here, as plain statements (not inside a
    # substitution), means every `_system_*_get` call below -- even the ones
    # that run inside `$( )` -- merely READS the already-populated cache
    # (their own _BUILT guards see the work is done and return immediately)
    # instead of re-collecting once per getter. One static pass + one
    ...
    # XACA-1091-004: sibling keys inside the SAME "system" object XACA-1031
    # authors above -- not a second "system" literal (EPIC-0061 Decision 8:
    # exactly ONE container). Every getter here reads the caches primed at
    # the top of this function; per the absence convention (contract §3) a
    # getter that printed nothing means "omit this key", never null/""/0.
    # Static, string-typed leaves -- json_escape'd like every other string
    # field in this function, omitted when the getter came back empty.
    local os_version_val="" os_build_val="" os_name_val="" model_val="" arch_val=""
    os_version_val="$(_system_static_get os_version)"
    os_build_val="$(_system_static_get os_build)"
    os_name_val="$(_system_static_get os_name)"
    model_val="$(_system_static_get model)"
    arch_val="$(_system_static_get arch)"

    # Static, integer-typed leaves -- bare JSON numbers, never quoted strings.
    local cores_val="" total_ram_val="" boot_time_val=""
    cores_val="$(_system_static_get cores)"
    total_ram_val="$(_system_static_get total_ram)"
    boot_time_val="$(_system_static_get boot_time)"

    local system_extra_json=""
    [ -n "$os_version_val" ] && system_extra_json="${system_extra_json},\"os_version\":\"$(json_escape "$os_version_val")\""
    [ -n "$os_build_val" ] && system_extra_json="${system_extra_json},\"os_build\":\"$(json_escape "$os_build_val")\""
    [ -n "$os_name_val" ] && system_extra_json="${system_extra_json},\"os_name\":\"$(json_escape "$os_name_val")\""
    [ -n "$model_val" ] && system_extra_json="${system_extra_json},\"model\":\"$(json_escape "$model_val")\""
    [ -n "$arch_val" ] && system_extra_json="${system_extra_json},\"arch\":\"$(json_escape "$arch_val")\""
    # NOTE: test with `[ -n ]` (emptiness), never `[ "$x" != 0 ]`-style
    # truthiness -- a COLLECTED zero (e.g. cores never legitimately 0, but
    # the same rule governs swap/pressure/load below) is real data.
    [ -n "$cores_val" ] && system_extra_json="${system_extra_json},\"cores\":${cores_val}"
    [ -n "$total_ram_val" ] && system_extra_json="${system_extra_json},\"total_ram\":${total_ram_val}"
    [ -n "$boot_time_val" ] && system_extra_json="${system_extra_json},\"boot_time\":${boot_time_val}"

    # Volatile leaves. "memory" and "disk" are nested objects that are
    # OMITTED ENTIRELY when nothing in the group could be collected (contract
    # §2/§3), unlike "system"/"versions" which are always emitted -- so build
    # each group's inner fields first and only wrap it in an object if at
    # least one field survived.
    local memory_used_val="" memory_total_val="" memory_pressure_val=""
    memory_used_val="$(_system_volatile_get memory_used)"
    memory_total_val="$(_system_volatile_get memory_total)"
    memory_pressure_val="$(_system_volatile_get memory_pressure_percent)"

    local memory_fields=""
    [ -n "$memory_used_val" ] && memory_fields="${memory_fields}\"used\":${memory_used_val},"
    [ -n "$memory_total_val" ] && memory_fields="${memory_fields}\"total\":${memory_total_val},"
    [ -n "$memory_pressure_val" ] && memory_fields="${memory_fields}\"pressure_percent\":${memory_pressure_val},"
    if [ -n "$memory_fields" ]; then
        memory_fields="${memory_fields%,}"
        system_extra_json="${system_extra_json},\"memory\":{${memory_fields}}"
    fi

    # swap_used_bytes -- ABSOLUTE BYTES (contract §4, non-negotiable). A
    # collected 0 means "not swapping" -- the healthiest reading -- and IS
    # emitted; `[ -n ]` tests emptiness, not truthiness, so a real 0 survives.
    local swap_used_val=""
    swap_used_val="$(_system_volatile_get swap_used_bytes)"
    [ -n "$swap_used_val" ] && system_extra_json="${system_extra_json},\"swap_used_bytes\":${swap_used_val}"

    local disk_used_val="" disk_free_val="" disk_percent_val=""
    disk_used_val="$(_system_volatile_get disk_used)"
    disk_free_val="$(_system_volatile_get disk_free)"
    disk_percent_val="$(_system_volatile_get disk_percent)"

    local disk_fields=""
    [ -n "$disk_used_val" ] && disk_fields="${disk_fields}\"used\":${disk_used_val},"
    [ -n "$disk_free_val" ] && disk_fields="${disk_fields}\"free\":${disk_free_val},"
    [ -n "$disk_percent_val" ] && disk_fields="${disk_fields}\"percent\":${disk_percent_val},"
    if [ -n "$disk_fields" ]; then
        disk_fields="${disk_fields%,}"
        system_extra_json="${system_extra_json},\"disk\":{${disk_fields}}"
    fi

    # load_average -- JSON array of 3 floats (1m/5m/15m), never a string.
    # _system_volatile_get returns the three tokens space-separated (or
    # nothing at all -- the getter never returns 1 or 2 of the 3; the
    # collector writes the record only once all three parsed).
    local load_average_val=""
    load_average_val="$(_system_volatile_get load_average)"
    if [ -n "$load_average_val" ]; then
        local la_l1="" la_l2="" la_l3=""
        read -r la_l1 la_l2 la_l3 <<_SYSTEM_LOAD_EMIT_EOF
$load_average_val
_SYSTEM_LOAD_EMIT_EOF
        if [ -n "$la_l1" ] && [ -n "$la_l2" ] && [ -n "$la_l3" ]; then
    ...
    cat <<EOF
{
  ...
  "system": {
    "schema_version": 1,
    "versions": $versions_json$system_extra_json
  }
}
EOF
}
