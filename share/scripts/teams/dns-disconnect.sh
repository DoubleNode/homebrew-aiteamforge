...
# `-a -x iTerm2` (not `-f "iTerm.app"`): this script runs INSIDE the iTerm2
# pane being closed, and macOS pgrep excludes the caller's ancestors unless
# -a is given, so a plain pgrep can never see its own host (XACA-1340/1341).
if [[ -d "/Applications/iTerm.app" ]] && pgrep -a -x iTerm2 > /dev/null; then
    if [[ -n "$HOST_FILTER" ]]; then
