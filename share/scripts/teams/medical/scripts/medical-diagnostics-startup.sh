...
# Primary Developer: Dr. Gregory House (Lead Feature Developer)
# Color Theme: Command Blue (representing diagnostic authority)

SESSION_THEME="COMMAND"
SESSION_TYPE="medical"
SESSION_NAME="diagnostics"
SESSION_DESCRIPTION="DIAGNOSTIC MEDICINE - LEAD DEVELOPER"
SESSION_LOCATION="Medical: Diagnostic Medicine Department"
SESSION_DEVELOPER="Dr. Gregory House"
SESSION_ROLE="Lead Feature Developer & Chief Diagnostician"
...
if [ $? != 0 ]; then
    clear
    echo "  Initializing Medical Diagnostics Terminal..."

    for i in {0..3}; do
        TERMINAL_NUMBER=$i
        case $i in
            0) TERMINAL_NAME="diagnostics-cmd"; TERMINAL_DESCRIPTION="Diagnostic Command Center" ;;
            1) TERMINAL_NAME="differential"; TERMINAL_DESCRIPTION="Differential Diagnosis & Analysis" ;;
            2) TERMINAL_NAME="case-review"; TERMINAL_DESCRIPTION="Case Review & Feature Planning" ;;
            3) TERMINAL_NAME="whiteboard"; TERMINAL_DESCRIPTION="Whiteboard Sessions & Brainstorming" ;;
        esac
    ...
    # Configure tmux status line - Command Blue theme (diagnostic authority)
    ...
    $TMUX_CMD set -t $SESSION_CODE @claude_agent "house"
    ...
    echo "  Medical Diagnostics Terminal initialized"
    echo ""
    echo "--> 4 diagnostic stations active"
    echo "--> Dr. House reporting for duty"
    echo ""
    sleep 1
fi
