...
# Color Theme: Medical Blue/White
...
LCARS_TEAM="medical-${PROJECT_LOWER}"
LCARS_PORT="$MEDICAL_LCARS_PORT"
...
if [ $? != 0 ]; then
    clear
    echo "  Initializing Medical LCARS Display..."
    ...
    # Configure tmux status line - Medical theme (blue/white - representing healthcare)
    $TMUX_CMD set -t $SESSION_CODE status-left-length 15
    $TMUX_CMD set -t $SESSION_CODE status-left "  LCARS "
    $TMUX_CMD set -t $SESSION_CODE @developer "Medical Team"
    ...
    $TMUX_CMD set -t $SESSION_CODE status-left-style "bg=colour231,fg=colour25,bold"
    ...
    $TMUX_CMD set -t $SESSION_CODE window-status-current-style "bg=colour231,fg=colour25,bold"
    $TMUX_CMD set -t $SESSION_CODE pane-border-style "fg=colour25"
    $TMUX_CMD set -t $SESSION_CODE pane-active-border-style "fg=colour231"
    ...
    echo "  Medical LCARS Display initialized"
    echo ""
