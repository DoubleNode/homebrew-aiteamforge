...
# Primary Developer: Dr. Allison Cameron (Lead Tester/QA)
# Color Theme: Sciences Blue (representing precision and scientific rigor)

SESSION_THEME="SCIENCES"
SESSION_TYPE="medical"
SESSION_NAME="immunology"
SESSION_DESCRIPTION="IMMUNOLOGY - LEAD TESTER/QA"
SESSION_LOCATION="Medical: Immunology Department"
SESSION_DEVELOPER="Dr. Allison Cameron"
SESSION_ROLE="Lead Tester & Quality Assurance"
...
if [ $? != 0 ]; then
    clear
    echo "  Initializing Medical Immunology Terminal..."

    for i in {0..3}; do
        TERMINAL_NUMBER=$i
        case $i in
            0) TERMINAL_NAME="immunology-cmd"; TERMINAL_DESCRIPTION="Immunology Command Center" ;;
            1) TERMINAL_NAME="test-suite"; TERMINAL_DESCRIPTION="Test Suite & Unit Testing" ;;
            2) TERMINAL_NAME="integration"; TERMINAL_DESCRIPTION="Integration Testing & QA" ;;
            3) TERMINAL_NAME="coverage"; TERMINAL_DESCRIPTION="Coverage Analysis & Reporting" ;;
        esac
    ...
    # Configure tmux status line - Sciences Blue theme (precision and rigor)
    ...
    $TMUX_CMD set -t $SESSION_CODE @claude_agent "cameron"
    ...
    echo "  Medical Immunology Terminal initialized"
    echo ""
    echo "--> 4 immunology stations active"
    echo "--> Dr. Cameron reporting for duty"
    echo ""
    sleep 1
fi
