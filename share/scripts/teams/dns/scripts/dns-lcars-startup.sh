#!/usr/bin/env bash
set +x
# DNS Framework LCARS Terminal Startup
# Lower Decks Location: Main Computer Interface
# Primary Function: Kanban Workflow Monitor
# Color Theme: Operations Gold (Lower Decks)
...
# Port for DNS LCARS (matches Fleet Monitor config)
LCARS_PORT=8180
...
if [ $? != 0 ]; then
    clear
    echo "  Initializing DNS LCARS Display..."
    ...
    # Configure tmux status line - Lower Decks Gold theme
    $TMUX_CMD set -t $SESSION_CODE status-left-length 15
    $TMUX_CMD set -t $SESSION_CODE status-left "  LCARS "
    $TMUX_CMD set -t $SESSION_CODE @developer "USS Cerritos"
    ...
    if [ -z "$SKIP_SERVER_START" ]; then
        $TMUX_CMD send-keys -t $SESSION_CODE:0 "cd $SESSION_DIRECTORY/lcars-ui && LCARS_TEAM=dns LCARS_SESSION_NAME=$SESSION_CODE python3 server.py $LCARS_PORT" C-m
    else
    ...
    echo "  DNS LCARS Display initialized"
    echo ""
