#!/bin/zsh
# Finance Team Terminal Banner Display Script
# Usage: finance-banner.sh '$SESSION_THEME' '$SESSION_TYPE' '$SESSION_NAME' '$TERMINAL_NUMBER' '$TERMINAL_NAME' '$SESSION_DESCRIPTION' '$SESSION_LOCATION' '$SESSION_DEVELOPER' '$SESSION_ROLE' '$TERMINAL_DESCRIPTION' '$SESSION_CODE'

SESSION_THEME="${1:-Finance Session Theme}"
SESSION_TYPE="${2:-Finance Session Type}"
SESSION_NAME="${3:-Finance Session Name}"
TERMINAL_NUMBER="${4:-Finance Terminal Number}"
TERMINAL_NAME="${5:-Finance Terminal Name}"
SESSION_DESCRIPTION="${6:-Finance Terminal Name}"
SESSION_LOCATION="${7:-FINANCE SESSION_LOCATION}"
...
# Command Gold (Grand Nagus Zek - Financial Strategy) - for finance-nagus
COMMAND_GOLD='%F{220}'             # Bright gold
COMMAND_GOLD_DARK='%F{178}'        # Deeper gold for highlights

# Operations Orange (Commerce & Transactions) - for finance-bar, finance-vault, finance-fca
OPS_ORANGE='%F{208}'               # Commerce orange
OPS_ORANGE_DARK='%F{166}'          # Darker orange for contrast

# Sciences Green (Data & Automation) - for finance-workshop
SCIENCES_GREEN='%F{34}'            # Deep green (latinum green)
SCIENCES_GREEN_BRIGHT='%F{40}'     # Brighter green

if [[ $SESSION_THEME == "COMMAND" ]]
then
THEME_COLOR=$COMMAND_GOLD
THEME_COLOR_HIGHLIGHT=$COMMAND_GOLD_DARK
fi
if [[ $SESSION_THEME == "OPERATIONS" ]]
then
THEME_COLOR=$OPS_ORANGE
THEME_COLOR_HIGHLIGHT=$OPS_ORANGE_DARK
fi
...
display_agent_avatar "finance" "$SESSION_DEVELOPER"
