#!/usr/bin/env bash
set +x
# Legal Team Filings Terminal Startup
# Location: Court Filing Office
# Primary Developer: Brad Chase (Court Filings & Motions)
# Color Theme: Operations Gold (administrative work)

SESSION_THEME="OPERATIONS"
SESSION_TYPE="legal"
SESSION_NAME="filings"
SESSION_DESCRIPTION="COURT FILINGS - MOTIONS & PLEADINGS"
SESSION_LOCATION="Legal: Court Filing Office"
SESSION_DEVELOPER="Brad Chase"
SESSION_ROLE="Filing Specialist & Motion Drafter"
...
set_window_metadata() {
    case "$1" in
        0) TERMINAL_NUMBER=0; TERMINAL_NAME="filings-cmd"; TERMINAL_DESCRIPTION="Filings Command Center" ;;
        1) TERMINAL_NUMBER=1; TERMINAL_NAME="motions"; TERMINAL_DESCRIPTION="Motion Drafting" ;;
        2) TERMINAL_NUMBER=2; TERMINAL_NAME="pleadings"; TERMINAL_DESCRIPTION="Pleadings Preparation" ;;
        3) TERMINAL_NUMBER=3; TERMINAL_NAME="court-docs"; TERMINAL_DESCRIPTION="Court Document Management" ;;
    esac
}
...
if [ $? != 0 ]; then
    clear
    echo "  Initializing Legal Filings Terminal..."
    ...
    $TMUX_CMD set -t $SESSION_CODE @claude_agent "courtclerk"
    ...
    echo "  Legal Filings Terminal initialized"
    echo ""
    echo "--> 4 filings stations active"
    echo "--> Brad Chase reporting for duty"
    echo ""
    sleep 1
fi
