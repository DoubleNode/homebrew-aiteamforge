#!/bin/bash
set +x
# Legal Team Discovery Terminal Startup
# Location: Evidence Review Room
# Primary Developer: Jerry Espenson (Discovery & Evidence Analysis)
# Color Theme: Operations Gold (investigative work)

SESSION_THEME="OPERATIONS"
SESSION_TYPE="legal"
SESSION_NAME="discovery"
SESSION_DESCRIPTION="DISCOVERY - EVIDENCE REVIEW"
SESSION_LOCATION="Legal: Evidence Review Room"
SESSION_DEVELOPER="Jerry Espenson"
SESSION_ROLE="Discovery Specialist & Evidence Analyst"
...
set_window_metadata() {
    case "$1" in
        0) TERMINAL_NUMBER=0; TERMINAL_NAME="discovery-cmd"; TERMINAL_DESCRIPTION="Discovery Command Center" ;;
        1) TERMINAL_NUMBER=1; TERMINAL_NAME="document-review"; TERMINAL_DESCRIPTION="Document Review & Analysis" ;;
        2) TERMINAL_NUMBER=2; TERMINAL_NAME="evidence-catalog"; TERMINAL_DESCRIPTION="Evidence Cataloging" ;;
        3) TERMINAL_NUMBER=3; TERMINAL_NAME="interrogatories"; TERMINAL_DESCRIPTION="Interrogatory Management" ;;
    esac
}
...
if [ $? != 0 ]; then
    clear
    echo "  Initializing Legal Discovery Terminal..."
    ...
    $TMUX_CMD set -t $SESSION_CODE @claude_agent "paralegal"
    ...
    echo "  Legal Discovery Terminal initialized"
    echo ""
    echo "--> 4 discovery stations active"
    echo "--> Jerry Espenson reporting for duty"
    echo ""
    sleep 1
fi
