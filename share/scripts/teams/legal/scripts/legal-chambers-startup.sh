...
# Primary Developer: Denny Crane (Lead Counsel - Strategic Legal Direction)
# Color Theme: Command Blue (representing judicial authority)

SESSION_THEME="COMMAND"
SESSION_TYPE="legal"
SESSION_NAME="chambers"
SESSION_DESCRIPTION="JUDGES CHAMBERS - LEAD COUNSEL"
SESSION_LOCATION="Legal: Judge's Chambers"
SESSION_DEVELOPER="Denny Crane"
SESSION_ROLE="Lead Counsel & Case Strategist"
...
set_window_metadata() {
    case "$1" in
        0) TERMINAL_NUMBER=0; TERMINAL_NAME="chambers-cmd"; TERMINAL_DESCRIPTION="Lead Counsel Command Center" ;;
        1) TERMINAL_NUMBER=1; TERMINAL_NAME="case-strategy"; TERMINAL_DESCRIPTION="Case Strategy & Planning" ;;
        2) TERMINAL_NUMBER=2; TERMINAL_NAME="client-comm"; TERMINAL_DESCRIPTION="Client Communications" ;;
        3) TERMINAL_NUMBER=3; TERMINAL_NAME="team-coord"; TERMINAL_DESCRIPTION="Legal Team Coordination" ;;
    esac
}
...
if [ $? != 0 ]; then
    clear
    echo "  Initializing Legal Chambers Terminal..."
    ...
    # Configure tmux status line - Command Blue theme (judicial authority)
    ...
    $TMUX_CMD set -t $SESSION_CODE @claude_agent "advocate"
    ...
    echo "  Legal Chambers Terminal initialized"
    echo ""
    echo "--> 4 chambers stations active"
    echo "--> Denny Crane reporting for duty"
    echo ""
    sleep 1
fi
