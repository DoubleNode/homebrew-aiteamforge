...
### Added
- fix: XACA-0886 — closed remaining protected-subitem bypasses in kb-cancel / kb-backlog sub cancel|remove found in PR #881 review: `kb-backlog sub remove` and `sub rename` now refuse a protected [Review]/[Test]/[UX] subitem the same way `kb-cancel`/`sub cancel` already did (shared `_kb_protected_tag_of` tag detector + `_kb_protected_cancel_guard`, reused rather than a second tag check); `kb-cancel`'s subitem branch refuses a nonexistent subitem ID instead of a false "Task cancelled!"; cancelling an ITEM (with or without `--force`) is now refused while it still has an OPEN protected subitem, overridable only via `--user-approved` + `--reason`; `--` now ends option parsing so a reason beginning with `--` is accepted positionally, and `--reason` immediately followed by a recognized flag errors instead of swallowing it. Template port: `_kb_get_activity_dir` gains a `KB_ACTIVITY_DIR` sandbox override and `_kb_release_sync` gains a `KB_SKIP_RELEASE_SYNC` escape hatch (test-suite sandboxing only; default behavior unchanged). `kb-backlog sub rename` (title-rename) does not exist in this tap template (long-diverged; not ported here — nothing to change).
