...
def main():
    ...
    try:
        ...
        print("     (POST is state-mutating and gated per XACA-0395 — pass the")
        print("     key via X-API-Key: `kb-api-key show` prints the effective key,")
        print("     resolved with the same env-over-file precedence the server uses.")
        print("     Omit the header entirely if no key is provisioned; the gate is")
        print("     open until one is.)")
        print()
        print('     curl -X POST http://localhost:8080/api/integrations/create-item \\')
        print('       -H "Content-Type: application/json" \\')
        print('       -H "X-API-Key: $(kb-api-key show)" \\')
