...
class LCARSHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def serve_status(self):
        ...
        status = {
            ...
            # XACA-0988-019/020: _lcars_bind_status_for_api() — not a bare
            # dict(_LCARS_BIND_STATUS) — returns an independent "hosts" list
            # and masks tailscale_ip when mode=all; see its docstring.
            "bind": _lcars_bind_status_for_api(),
        }
...
def _lcars_bind_status_for_api():
    """Build the "bind" object serve_status() puts on /api/status.

    XACA-0988-020: `dict(_LCARS_BIND_STATUS)` alone is only a SHALLOW copy —
    its "hosts" value would be the exact same list object as the module
    global's, so any future code that mutated `status["bind"]["hosts"]` in
    place (e.g. `.append()` / `.sort()`) after calling this would corrupt
    what every other request handler, and the next
    resolve_bind_addresses_or_die() run, sees. Copy "hosts" into a fresh
    list explicitly rather than relying on dict()'s shallow semantics.

    XACA-0988-019: LCARS_BIND_MODE=all binds 0.0.0.0 — reachable from any
    device on the LAN, not just the tailnet — and /api/status has no auth
    gate of its own (XACA-0397's threat model relies on the bind ADDRESS,
    not a token, to keep unauthenticated LAN clients out; see the "mode=all"
    WARNING banner in resolve_bind_addresses_or_die()). Disclosing this
    server's tailnet address to such a client hands out tailnet topology to
    anyone who can merely reach the LAN, whether or not they are themselves
    a tailnet peer. resolve_bind_addresses_or_die()'s "all" branch does NOT
    populate tailscale_ip today (only the auto/tailscale branch does), so
    this masking is defensive rather than fixing an active leak — but that
    is exactly why it belongs HERE, at the API boundary, instead of resting
    on "the internal function happens not to set it": a future change to
    that branch (e.g. recording the host's tailscale IP for informational
    purposes even under mode=all) must not silently start leaking it again.
    Every other mode keeps the field: loopback/degraded-auto never populate
    it, and tailscale/detected-auto bind NARROWLY to [loopback, tailscale
    ip] — a caller able to reach that narrower bind already has tailnet or
    local-host access, so there is nothing new for the field to disclose.
    """
    status = dict(_LCARS_BIND_STATUS)
    status["hosts"] = list(_LCARS_BIND_STATUS.get("hosts") or [])
    if status.get("mode") == "all":
        status["tailscale_ip"] = None
    return status
