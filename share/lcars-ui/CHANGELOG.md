...
### Fixed
- **XACA-0629: Roadmap "Export PDF" rendered a blank page.** The print-scope rules (`body.roadmap-printing`) tried to reflow the absolutely-positioned LCARS shell to static flow, but the base section rule `.lcars-content > .roadmap-section { position: absolute !important }` (specificity 0,2,0 + `!important`) defeated the print rule's un-`!important` `position: static`. The section stayed `position:absolute` with `top:0;bottom:0` inside a now-auto-height container, collapsing to **0px height** → empty print output. Fix: raise the print rule's selector to `body.roadmap-printing .lcars-content > .roadmap-section` (specificity 0,3,1) and add `!important` + explicit `top/left/right/bottom:auto` resets so it beats both the base `position:absolute !important` and the `.active` `overflow-y:auto !important`; also override `#roadmap-content`'s `min-height:100%` and neutralize the lane scroll container so the full timeline renders at content height; and duplicate the chrome-hide/layout-reset block inside `@media print` (some engines apply print rules independently). Verified by rendering the print output to PDF (full timeline visible: axis, epic bars, unscheduled chips). Cache-buster bumped: `lcars.css?v=32.1→32.2`.
