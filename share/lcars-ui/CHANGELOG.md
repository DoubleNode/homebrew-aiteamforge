...
- **XACA-0948 (PR #761 review + UX round) — a sixth manifest-writing engine, an unguarded board read, two divergent team-less paths, and a status pill whose accessibility cue never rendered.**
  ...
  - `css/lcars.css`: `.status-unresolved` referenced the **undefined** `--lcars-muted` with no fallback, so its `border` shorthand was invalid at computed-value time and dropped entirely — the dashed-border cue that made the pill legible without relying on colour was never rendering. Now `var(--lcars-muted, #888)`; measured contrast 5.20:1 (WCAG 2.1 AA pass).
  - `js/lcars.js`: surfaces the server's `progress.unresolved` as an amber `(N unresolved)` suffix so a release's ratio moving is legible; adds an explanatory tooltip to the `UNRESOLVED` pill; normalizes the display label's underscores to spaces while the CSS class keeps the raw token. Cache-busters `lcars.css?v=32.27→32.28`, `lcars.js?v=3.61→3.62`.

- **XACA-0948: `manifest.items[].status` deprecated — release-item status is now resolved read-time from the board, per `ITEM_STATUS_CONTRACT.md` §1.5.** Persisted status rotted: 13.6% of live manifest rows disagreed with board truth, including the incident that motivated this ticket (a manifest still showing `in_progress` for items the board had marked `completed`). `_derive_item_status` is renamed `_resolve_item_status` and rewritten to the contract's precedence (recorded status always wins verbatim, including non-canonical tokens; an unrecorded item resolves from subitem/activity evidence with a hard ceiling of `in_progress` — never a terminal state or `blocked`).
  - **Writers** (`handle_assign_item_to_release`, `_sync_item_to_release_manifest`) no longer compute or persist a `status` field; a legacy value is lazily stripped the moment a row is touched.
  - **Readers** (`serve_release_items`, `serve_items_by_release`) now share one resolver (`_resolve_release_items`) that re-resolves every row from its own team's board on every request; an unresolvable row reports `status: null` / `statusResolution: "unresolved"` rather than a stale or fabricated value.
  - **`_calculate_release_progress`** drops its manifest fallback and now resolves each row from its *own* team's board (fixing a pre-existing bug where cross-team rows only ever resolved against the serving team's board and silently used the stale persisted value); unresolvable rows are excluded from `completed` and surfaced in a new `unresolved` counter — academy's release-progress numbers may move as a result of this becoming visible.
  - **Frontend:** `lcars.js`'s release-items panel guards against a missing/unresolved/non-canonical status (previously `item.status.toUpperCase()` was unguarded and a single such row threw, breaking the whole panel with "Error loading items"). `lcars.css` gains `.status-blocked` / `.status-in_review` / `.status-unknown` / `.status-unresolved`. Cache-busters: `lcars.css?v=32.26`→`32.27`, `lcars.js?v=3.60`→`3.61`.
  - `scripts/sync-release-manifests.py` (a fifth, previously-undocumented status-writing engine) no longer writes `status` either.
  - See `kanban/plans/XACA-0948/` for the full contract, decision record, and evidence.
...
### Fixed
- **XACA-0633: Roadmap "Export PDF" now generates a PDF client-side instead of `window.print()`.** LCARS runs inside iTerm2's WKWebView cockpit, where `window.print()` is a no-op — clicking Export added the `body.roadmap-printing` scope, no print dialog ever opened, and `afterprint` never fired, leaving the UI stuck in a full-window takeover with no PDF. Reworked `exportRoadmapPdf()` to capture `#roadmap-content` with **html2canvas** and build a landscape PDF with **jsPDF** (both vendored locally at `js/vendor/`, no runtime/CDN/server deps), downloaded via `doc.save()` with a new-tab `bloburl` fallback for WebViews that don't honor `<a download>`. No more `window.print()`, no body-class takeover. Output is rasterized (JPEG-embedded — jsPDF 2.5.1's PNG parser rejects some html2canvas output). Verified by generating a real PDF headlessly (full timeline + unscheduled lanes captured). The legacy `body.roadmap-printing` / `@media print` CSS is left in place (harmless; still useful if LCARS is opened in a real browser). Cache-buster bumped `lcars.js?v=3.25→3.28`; vendor scripts added.

<!-- XACA-0629: Bugfix — Roadmap PDF export rendered blank -->

### Fixed
- **XACA-0629: Roadmap "Export PDF" rendered a blank page.** The print-scope rules (`body.roadmap-printing`) tried to reflow the absolutely-positioned LCARS shell to static flow, but the base section rule `.lcars-content > .roadmap-section { position: absolute !important }` (specificity 0,2,0 + `!important`) defeated the print rule's un-`!important` `position: static`. The section stayed `position:absolute` with `top:0;bottom:0` inside a now-auto-height container, collapsing to **0px height** → empty print output. Fix: raise the print rule's selector to `body.roadmap-printing .lcars-content > .roadmap-section` (specificity 0,3,1) and add `!important` + explicit `top/left/right/bottom:auto` resets so it beats both the base `position:absolute !important` and the `.active` `overflow-y:auto !important` (the section's `.active` `overflow-y:auto` scroll is reset via `overflow-y:visible`); reset `min-height:0` on the section's ancestor chain (`.lcars-main`/`.lcars-content`) plus an explicit `body.roadmap-printing #roadmap-content { min-height:0 }` override (the base `#roadmap-content` rule carries `min-height:100%`) so the content area is content-driven; and duplicate the chrome-hide/layout-reset block inside `@media print` (some engines apply print rules independently). Verified by rendering the print output to PDF (full timeline visible: axis, epic bars, unscheduled chips). Cache-buster bumped: `lcars.css?v=32.1→32.3`.
