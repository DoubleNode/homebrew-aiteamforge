...
3. **`crState`** — CR lifecycle state (`cr-drafted`, `cr-published`, `cr-submitted`, `cr-approved`, `implementing`, `deployed-dev`, `deployed-prod`, `emergency-deployed`, `cr-closed`, …). Added by XACA-0291. `cr-published` (rank 5, between `cr-drafted`=0 and `cr-submitted`=10) was added by XACA-0895 to split "local draft, nothing published" from "Confluence page live, CR-Proper link not yet appended" — previously both conditions were carried by `cr-drafted` alone. `cr-closed` is a manual terminal archive state (XACA-0349) — reachable from any state via `kb-cr close`; does not increment `pushback_count`.
