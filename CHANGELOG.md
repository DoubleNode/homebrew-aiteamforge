...
- XACA-0639: Roadmap archived-Release activity span — archived Releases now surface a historical bar on the scheduled timeline instead of always dropping to the unscheduled Archived bucket (`share/lcars-ui/server.py`, `share/lcars-ui/js/lcars.js`, `share/lcars-ui/index.html`; cache-buster `lcars.js?v=3.39`→`?v=3.40`). Follow-up to XACA-0638/0635 (EPIC-0040). `serve_roadmap` builds a release→items index from the inverse pointer `item.releaseAssignment.releaseId` (releases carry no itemIds), then a new `_schedule_archived_release(release)` helper derives the span: PRIMARY = item-activity rollup (reuses `_activity_rollup`, now parameterized with `extend_to_today=True` default so epic behavior is unchanged) over the release's assigned items with `extend_to_today=False` (archived releases are historical — never clamp to today even with open items); FALLBACK = the release's own `createdAt → archivedAt` lifecycle span when no completed-item timestamps exist; releases with neither stay unscheduled. Emits `scheduleSource='activity'`, and the release timeline-bar render path now renders the muted-green `ACTUAL` badge (previously releases had no schedule-source badge). Applied to BOTH archived-release sites (on-board `status='archived'` + the `_load_archived_releases` archive-dir loop); dedup-by-id preserved. Scope: archived releases only — non-archived releases keep their `targetDate` point-event behavior. Mirror of canonical `lcars-ui/` changes via `sync-tap.sh`.
