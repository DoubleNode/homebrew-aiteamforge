...
- XACA-0647: Canonical-guard no longer flags personal-only machine configs (academy + personal teams such as finance-personal/legal-coparenting/medical-general, no platform team) as corrupt; only academy-alone/empty triggers re-seed. The old heuristic demanded academy PLUS at least one of ios/android/firebase/dns, wrongly re-seeding fleet-plan personal machines with the Main Event roster. Fixed in both `share/kanban-hooks/aiteamforge_paths.py` (`has_non_required` replaces `has_canonical_one`) and `share/lcars-ui/server.py` (`has_non_required` replaces `has_at_least_one`); platform-team constants retained for documentation. Canonical change in `dev-team`; mirror via `sync-tap.sh`. (XACA-0457 follow-on)
