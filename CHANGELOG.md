...
- XACA-0666: Auto-deploy personas into multi-project personal-team project dirs at startup (`share/scripts/teams/legal-startup.sh`, `share/scripts/teams/finance-startup.sh`, `share/scripts/teams/medical-startup.sh`; mirror of the canonical root `<team>-startup.sh`). The legal/finance/medical teams `cd` into `~/<team>/<PROJECTID>` (a nested git repo), but their persona manifest target is the umbrella parent (`~/<team>`), so personas were deployed one level above the project git root — where Claude Code never looks — and sessions loaded zero personas. Each `*-startup.sh` now runs a guarded, non-fatal `kb-sync-personas sync-worktrees <team>` immediately after `kb_ensure_team_initialized`, invoking the existing XACA-0660 nested-root deploy so the active project dir's `.claude/agents` is populated/refreshed at launch (kept untracked via `.git/info/exclude`). Covers new projects and master-persona drift automatically. The guard (`[ -x ... ]` + `|| echo`) makes it a safe no-op where `kb-sync-personas` is absent. `kb-sync-personas` itself is dev-team-only (not tap-mirrored). Mirror of canonical root `<team>-startup.sh` changes via `sync-tap.sh`.
