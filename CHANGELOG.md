...
- XACA-0649: Consumer shell resolver `_kb_get_kanban_dir` (`share/templates/aliases/kanban-aliases.sh`) no longer mis-routes registered parameterized teams to `$AITEAMFORGE_DIR/kanban`. An early-exit guard `if [[ -d "${_atf_dir}/kanban" ]]` in the old code fired for ALL teams once the academy board had created that directory on disk — so `legal-coparenting` was routed to `~/aiteamforge/kanban` instead of `~/legal/coparenting/kanban`, causing `kb-quarantine-stub legal-coparenting` to fail with "no canonical board found". Fixed with three-tier lookup: (1) `team-paths.json` registry first (inline python3 reads the same `kanban_dir` the Python server uses — ends shell/server resolver divergence); (2) well-known case arms (parameterized families derive from team-slug suffix, not `$AITEAMFORGE_DIR`); (3) `$AITEAMFORGE_DIR/kanban` catch-all for genuinely unknown teams only. `kb-init-team-guard.sh` (`share/scripts/kb-init-team-guard.sh`, mirrored via `sync-tap.sh`) gains `_kb_detect_and_quarantine_stub`: on startup, if a legacy `<template>-default` stub coexists with the canonical board it auto-delegates to `kb-quarantine-stub --yes` (stash, not delete) so first-launch operators no longer hit a manual FATAL wall. Regression test: `tests/test-xaca-0649-kanban-dir-resolver.sh` (7 cases, including the exact bug scenario).
