...
- XACA-0629: Bugfix — Roadmap "Export PDF" rendered a blank page (`share/lcars-ui/css/lcars.css`, `share/lcars-ui/index.html`; cache-buster bumped `lcars.css?v=32.1`→`?v=32.2`). The `body.roadmap-printing` print scope failed to override the base `.lcars-content > .roadmap-section { position:absolute !important }` (un-`!important` `position:static` lost on specificity), so the section stayed absolute with `top:0;bottom:0` on an auto-height parent and collapsed to 0px → empty print output. Fix raises the print selector specificity (`body.roadmap-printing .lcars-content > .roadmap-section`) + `!important` + offset resets, overrides `#roadmap-content` `min-height` and the lane scroll container, and duplicates the layout-reset block inside `@media print`. Verified by rendering the print output to PDF. Mirror of canonical lcars-ui changes via `sync-tap.sh`.
