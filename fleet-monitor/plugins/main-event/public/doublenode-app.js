/**
 * DoubleNode Operations Dashboard - Client Application
 * Filtered view showing DoubleNode divisions plus Academy
 *
 * Divisions: dns, freelance, academy
 */
...
// DoubleNode Division Filter (includes Academy for dev team visibility)
const DOUBLENODE_DIVISIONS = ['dns', 'freelance', 'academy'];
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('DoubleNode Operations Dashboard initializing...');
    fetchFleetData();
...
async function fetchFleetData() {
    try {
        ...
        const filteredData = filterDoubleNodeData(fleetData);
...
function filterDoubleNodeData(data) {
    ...
    for (const [divisionName, divisionData] of Object.entries(fleet.divisions || {})) {
        if (DOUBLENODE_DIVISIONS.includes(divisionName.toLowerCase())) {
    ...
    const filteredMachines = (fleet.machines || []).map(machine => {
        const filteredSessions = (machine.sessions || []).filter(session =>
            DOUBLENODE_DIVISIONS.includes((session.division || '').toLowerCase())
        );
        return {
            ...machine,
...
function renderDivisions(divisions) {
    ...
    if (!divisions || Object.keys(divisions).length === 0) {
        container.innerHTML = '<p style="text-align: center; color: var(--text-secondary); padding: 40px;">No active DoubleNode divisions detected</p>';
        return;
    }
