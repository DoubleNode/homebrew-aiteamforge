...
(function() {
    ...
    const CONFIG = {
        ...
        divisions: ['finance-personal'],
        dashboardName: 'FERENGI COMMERCE AUTHORITY',
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('[LCARS] Finance Dashboard initializing...');
        ...
        console.log('[LCARS] Finance Dashboard initialized');
    });
    ...
    async function fetchEpics() {
        try {
            // Fetch epics for the first division (finance-personal)
    ...
    function createTeamCard(name, data) {
        ...
        var avatarHtml = '';
        if (avatarUrl) {
            var divisionClass = session.division ? 'div-' + session.division.toLowerCase() : 'div-finance';
        ...
        if (epic.items && epic.items.length > 0) {
            epic.items.forEach(function(item) {
                ...
                if (item.persona || item.assignedTo) {
                    ...
                    var teamName = item.team || epic.team || 'finance-personal';
    ...
    function createMachineItem(machine) {
        ...
        if (machineDivisions.length > 0 && backupStatus && backupStatus.boards) {
            ...
            function processBackupBoard(boardName, teamBackup) {
                ...
                // Format display name (e.g., "finance-personal" -> "FINANCE-PERSONAL")
            ...
            machineDivisions.forEach(function(division) {
                ...
                // Also find all boards that start with this division (e.g., finance-*)
    ...
    function getDivisionTitle(divisionCode) {
        ...
        const titles = {
            'finance-personal': 'FERENGI COMMERCE AUTHORITY',
        ...
        // Fallback for any finance-* variant not explicitly listed
        if (!titles[code] && code.startsWith('finance')) {
            const suffix = code.replace('finance-', '').toUpperCase();
            return 'FERENGI ' + suffix;
        }
    ...
    function getDivisionColorClass(divisionCode) {
        const code = divisionCode.toLowerCase();
        const colors = {
            'finance-personal': 'div-finance-personal',
            'ios': 'div-ios',
            'android': 'div-android',
        ...
        // Fallback for any finance-* variant not explicitly listed
        if (!colors[code] && code.startsWith('finance')) {
            return 'div-finance';
        }
        // Fallback for any freelance-* variant not explicitly listed
     ...
     * Session names follow patterns like "finance-personal-quark-lcars", etc.
    ...
    function getCharacterName(session) {
        ...
        const characters = [
            'quark-fin', 'rom', 'nog', 'brunt', 'zek',    // Finance/Ferengi
     ...
     * @param {string} division - Division/team name (e.g., 'finance-personal')
     * @param {string} character - Character name (e.g., 'quark', 'rom')
